...
# I recieved some help in class from Dr.Malone
...
def exchange(str):
    ...
    str_list = list(str)              #converts string to list

    first_char = str_list[0]          #swap
    str_list[0] = str_list[-1]
    str_list[-1] = first_char
    
    result_str = ''.join(str_list)    #joins list back to string
...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    ...
    alist = [val for val in alist if not (min <= val <= max)]     #keeps values not in the range of min and max
    return alist
...
def word_count_in_set(words):
    ...
    *** Solution should not be more than 3 lines of code (can be less)
    ...
    """
    return len(set(words))     #converts words to a set (no duplicates) and calculates the length(number of unique words)
...
def topping1(dictionary):
    ...
    if "ice cream" in dictionary:                #if key is present 
        dictionary['ice cream'] = 'cherry'       #set value of key
    dictionary['bread'] = 'butter'               #set value of key
    return dictionary                            
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """   
    return {val: strings.count(val) for val in set(strings)}    #convert strings to set, keys are the unique elements and the values are the counts
...
def friend_list(friend_dictionary):
    ...
    friend_lst = {}                                 #empty dictionary to store friendships
    for person, friend in friend_dictionary.items():
        if person in friend_lst:
            friend_lst[person].append(friend)
        else:
            friend_lst[person] = [friend]
        if friend in friend_lst:                    #friendships are bi-directional- check
            friend_lst[friend].append(person)
        else:
            friend_lst[friend] = [person]
    return friend_lst
